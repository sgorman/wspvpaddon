...
function Inspector:putTable(t)
  ...
  elseif self.depth and self.level >= self.depth then
    self:puts('{...}')
	  ...
	  curTable = t
	  --Print("curTable: " .. tostring(curTable))
...
function Inspector:putValue(v)
  ...
  elseif tv == 'table' then
    self:putTable(v)
--  elseif tv == 'function' and curTable ~= nil then
	--Print(curTable.GetName(curTable))
--	local suc, ret = pcall(v, curTable)
--    if suc then 
--		self:puts(tostring(ret)) 
--	else 
		--self:puts('<fail ',tv,' ',self:getId(v),'>')
--		self:puts(ret)
--	end
  else
...
_G["GeminiPackages"]:NewPackage(inspect,"drafto_inspect-1.0",3)